...
namespace smt::noodler {
    ...
    NielsenGraph NielsenDecisionProcedure::generate_from_formula(const Formula& init, bool early_termination, bool add_edges, bool & is_sat) const {
        ...
        while(!worklist.empty()) {
            ...
            //predicates with only one variable is trivial.
            ...
            for(const auto& label : rules) {
                ...
                if(is_pred_unsat(rpl.get_predicates()[index]) || is_length_unsat(rpl.get_predicates()[index])) {
                    continue;
                }
    ...
    bool NielsenDecisionProcedure::is_one_unsat(const Predicate& pred) {
        ...
        for(const auto& t : pred.get_vars()) {
            if(occurLeft[t] > occurRight[t]) n=occurLeft[t]-occurRight[t],m=occurRight[litTerm]-occurLeft[litTerm];
            if(occurLeft[t] < occurRight[t]) n=occurRight[t]-occurLeft[t],m=occurLeft[litTerm]-occurRight[litTerm];;  
        }
        ...
        while(lidx<left_side.size()&&ridx<right_side.size()){
            if(left_side[lidx].is_literal()&&right_side[ridx].is_literal()){
                ...
                ++lidx;
                ++ridx;
            }else if(left_side[lidx].is_variable()&&right_side[ridx].is_literal()){
                if(v[lpos].length()==0)v[lpos]=right_side[ridx].get_name(),++lpos;
                else if(v[lpos]!=right_side[ridx].get_name())return true;
                else{++lpos;}
                if(lpos==n)lpos=0, ++lidx;
                ++ridx;
            }else if(right_side[ridx].is_variable()&&left_side[lidx].is_literal()){
                if(v[rpos].length()==0)v[rpos]=left_side[lidx].get_name(),++rpos;
                else if(v[rpos]!=left_side[lidx].get_name())return true;
                else{++rpos;}
                if(rpos==n)rpos=0, ++ridx;
                ++lidx;
            }else{
                if(v[rpos].length()==0&&v[lpos].length()!=0)v[rpos]=v[lpos],++lpos,++rpos;
                else if(v[lpos].length()==0&&v[rpos].length()!=0)v[lpos]=v[rpos],++lpos,++rpos;
                else if(v[lpos]!=v[rpos])return true;
                else{
                    ++lpos,++rpos;
                }
                if(rpos==n)rpos=0, ++ridx;
        ...
        SASSERT(lidx==left_side.size()&&ridx==right_side.size());
        return false;
    }
    ...
    bool NielsenDecisionProcedure::is_length_unsat(const Predicate& pred) {
        ...
        std::map<zstring, unsigned> symbolLeft = pred.literal_count(Predicate::EquationSideType::Left);
        std::map<zstring, unsigned> symbolRight = pred.literal_count(Predicate::EquationSideType::Right);
        auto s2 = pred.get_literals();
        
        // todo: add check on each symbol maybe better
        for(const auto& t : s2) {
            if(allLeft && symbolLeft[t] < symbolRight[t]) {
                return true;
            }
            if(allRight && symbolRight[t] < symbolLeft[t]) {
                return true;
            }
        }
        ...
        //Bézout's lemma
        if(!tmp.empty()){
            size_t num = std::max(occurLeft[litTerm],occurRight[litTerm])-std::min(occurLeft[litTerm],occurRight[litTerm]);
            if(num%tmp[0]){
                return true;
            }
        }
