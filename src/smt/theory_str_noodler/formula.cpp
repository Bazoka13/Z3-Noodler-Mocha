...
namespace smt::noodler {
    ...
    std::set<zstring> Predicate::get_literals() const {
        std::set<zstring> vars;
        for (const auto& side: params) {
            for (const auto &term: side) {
                if (term.is_literal()) {
                    vars.insert(term.get_name());
                }
            }
        }
        return vars;
    ...
    std::string Predicate::to_string() const {
        ...
        if (is_equation()) {
            ...
            // result<< transducer.get()->print_to_dot()<<"\n";
        } else {
    ...
    std::map<zstring, unsigned> Predicate::literal_count(const Predicate::EquationSideType side) const {
        assert(is_eq_or_ineq());
        std::map<zstring, unsigned> occurr;
        unsigned lits = 0;
        for(const BasicTerm& b : this->get_side(side)) {
            if(b.is_variable()) {
                continue;
