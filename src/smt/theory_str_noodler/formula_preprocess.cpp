...
namespace smt::noodler {
    ...
    bool checkLoop(const mata::nfa::Nfa& input_nfa){
        ...
        if(!words.empty() && !words.begin()->empty()){
                for(const mata::Word & w :words){
                    if(w.empty())continue;
                    std::set<mata::Symbol> alphaz;
                    mata::EnumAlphabet mata_alphabet(alphaz.begin(),alphaz.end());
                    for(mata::Symbol s:w)
                        alphaz.insert(s);
                    mata::Symbol eps = mata::nfa::EPSILON;
                    mata::nfa::State ini = 0;
                    mata::nfa::State fin = 1;
                    mata::utils::SparseSet<mata::nfa::State> initial_states = {ini};
                    mata::utils::SparseSet<mata::nfa::State> final_states = {ini};
                    mata::nfa::Nfa nfa=mata::nfa::Nfa(1,initial_states,final_states,&mata_alphabet);
                    mata::Symbol pre = ini;
                    for(auto s:w){
                        if(nfa.num_of_states()!=alphaz.size()){
                            nfa.add_state();
                            nfa.delta.add(pre,s,nfa.num_of_states()-1);
                            pre = nfa.num_of_states()-1;
                        }else{
            ...
            for(size_t word_len=1;word_len<MAX_LEN;word_len++){
                auto words2 = input_nfa.get_words(word_len);
                if(!words2.empty() && words2.size()!=words.size()){
                    for(const mata::Word & w :words2){
                        if(w.empty())continue;
                        std::set<mata::Symbol> alphaz;
                        mata::EnumAlphabet mata_alphabet(alphaz.begin(),alphaz.end());
                        for(mata::Symbol s:w)
    ...
    bool FormulaPreprocessor::refine_pre_image() {
        std::set<size_t> rem_ids;
        for(const auto& pd: this->formula.get_predicates()){
            if(pd.second.is_transducer()){
                if(pd.second.get_output().size()==1&&pd.second.get_input().size()==1&&this->formula.single_occurr(pd.second.get_input_set())){
                    ...
                    mata::nfa::Nfa nfa_pre_image{ backward_applied_nft.to_nfa_move() };
                   
                    if(nfa_pre_image.final.empty())return true;
    ...
    void FormulaPreprocessor::refine_prefix_and_suffix() {
        ...
        for(const auto& pr : this->formula.get_predicates()) {
            ...
            std::vector<BasicTerm> right_vars = pr.second.get_right_side();
            size_t prefix_len = 0,suffix_len=0; 
            size_t num_left = left_vars.size(), num_right = right_vars.size();
            ...
            for(size_t pos = 0;pos<tot_num;pos++){
                ...
                size_t MAX_LEN = 10;
                if(!mata::nfa::are_equivalent((*this->aut_ass[right_vars[pos]]),(*this->aut_ass[left_vars[pos]])))
                    should_stop=true;    
                if(should_stop)
                    break;    
                //todo: need modify 
                if(pos>=1&&!should_stop){
                    if(!mata::nfa::are_equivalent((*this->aut_ass[left_vars[pos]]),(*this->aut_ass[left_vars[pos-1]])))
                        should_stop=true; 
                    if(!mata::nfa::are_equivalent((*this->aut_ass[right_vars[pos]]),(*this->aut_ass[right_vars[pos-1]])))
                        should_stop=true; 
                }
                if(should_stop)
                    break;
                if(!checkLoop((*this->aut_ass[left_vars[pos]])) || !checkLoop((*this->aut_ass[right_vars[pos]]))){
                    should_stop = true;
                }
                if(should_stop)
                    break;
                l_sum+=left_vars[pos].get_name().hash();
                r_sum+=right_vars[pos].get_name().hash();
                if(l_sum==r_sum)prefix_len=pos+1;
            }
            if(prefix_len<tot_num){
                l_sum = 0,r_sum=0;
                tot_num=std::min(num_left-prefix_len,num_right-prefix_len);
                for(size_t pos = 1;pos<=tot_num;pos++){
                    bool should_stop=false;
                    if(!mata::nfa::are_equivalent((*this->aut_ass[right_vars[num_right-pos]]),(*this->aut_ass[left_vars[num_left-pos]])))
                        should_stop=true;  
                    if(should_stop)
                        break;  
                    if(pos!=1&&!should_stop){
                        if(!mata::nfa::are_equivalent((*this->aut_ass[left_vars[num_left-pos]]),(*this->aut_ass[left_vars[num_left-pos+1]])))
                            should_stop=true; 
                        if(!mata::nfa::are_equivalent((*this->aut_ass[right_vars[num_right-pos]]),(*this->aut_ass[right_vars[num_right-pos+1]])))
                            should_stop=true; 
                    }
                    if(should_stop)
                        break;
                    if(!checkLoop((*this->aut_ass[left_vars[num_left-pos]])) || !checkLoop((*this->aut_ass[right_vars[num_right-pos]]))){
                        should_stop = true;
                    }
                    if(should_stop)
                        break;
                    l_sum+=left_vars[num_left-pos].get_name().hash();
                    r_sum+=right_vars[num_right-pos].get_name().hash();
                    if(l_sum==r_sum)suffix_len=pos;
                }
            }
            if(prefix_len!=0||suffix_len!=0){
                if(prefix_len!=0){
                    left_vars.erase(left_vars.begin(), left_vars.begin() + prefix_len);
                    right_vars.erase(right_vars.begin(), right_vars.begin() + prefix_len);
                }
                if(suffix_len!=0){
                    left_vars.erase(left_vars.begin() + (left_vars.size() - suffix_len), left_vars.end());
                    right_vars.erase(right_vars.begin() + (right_vars.size() - suffix_len), right_vars.end());
                }
                if(pr.second.is_equation()){
                    Predicate new_pred = Predicate::create_equation(left_vars,right_vars);
                    new_preds.insert(new_pred);
                }else{
                    Predicate new_pred = Predicate::create_disequation(left_vars,right_vars);
                    new_preds.insert(new_pred);
                }
                rem_ids.insert(pr.first);
            }
    ...
    Formula FormulaPreprocessor::get_modified_formula() const {
        Formula ret;
        std::vector<Predicate> tmp_pr;
        for(const Predicate& p: this->formula.get_predicates_set()) {
            if(p.is_transducer()){
                tmp_pr.push_back(p);
            }else ret.add_predicate(p);
        }
        if(!tmp_pr.empty()){
            size_t sz = tmp_pr.size();
            std::vector<bool> bits(sz+5);
            for(size_t pos = 0;pos<=sz;pos++)
                bits[pos]=false;
            for(size_t pos = 0;pos<sz;pos++){
                if(bits[pos])continue;
                bool fl = false;
                STRACE(str_prep,tout<<tmp_pr[pos].get_input().size()<<" pcp\n";);
                auto tmp_nft = *tmp_pr[pos].get_transducer();
                if(tmp_pr[pos].get_input().size()==1){
                    for(size_t posj=pos+1;posj<sz;posj++){
                        if(tmp_pr[pos].get_input_set()==tmp_pr[posj].get_input_set()){
                            if(tmp_pr[pos].get_output_set()==tmp_pr[posj].get_output_set()){
                                bits[posj]=1;
                                tmp_nft = mata::nft::intersection(tmp_nft,*tmp_pr[posj].get_transducer());
                                fl = true;
                            }
                        }
                    }
                }
                if(!fl)ret.add_predicate(tmp_pr[pos]);
                else{
