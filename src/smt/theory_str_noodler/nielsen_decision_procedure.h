...
namespace smt::noodler {
    ...
    protected:
        ...
        /**
         * @brief Check if the predicate with only one variable is unsatisfiable. It uses some kind of two-pointers.
         * 
         * @param pred Predicate to be checked
         * @return true -> unsatisfiable
         */
        static bool is_one_unsat(const Predicate& pred);
        ...
        static unsigned get_predicate_cost(const Predicate& pr) {
            // std::map<BasicTerm, unsigned> occurLeft = pr.variable_count(Predicate::EquationSideType::Left);
            // std::map<BasicTerm, unsigned> occurRight = pr.variable_count(Predicate::EquationSideType::Right);
            // BasicTerm litTerm(BasicTermType::Literal, "");
            return pr.get_left_side().size() + pr.get_right_side().size();
            
            // return (pr.get_left_side().size() + pr.get_right_side().size())/(pr.get_vars().size()+1);
            // return pr.get_vars().size();
            // -occurLeft[litTerm]-occurRight[litTerm];
        }
