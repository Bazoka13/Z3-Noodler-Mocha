...
namespace smt::noodler {
    ...
    */
    ...
    void theory_str_noodler::handle_replace(expr *r) {
        ...
        // STRACE(str, tout << "new var is " << mk_pp(v.get(), m) << '\n';);
        // replace!19 a="A" s=x t=(str.replace y "B" "A"))
    ...
    */
    void theory_str_noodler::add_kmp(const expr_ref& left_side,const expr_ref& right_side, const bool have_membership){
        ...
        if(num_r>=2&&util::is_str_variable(l_vec.get(0),m_util_s) &&m_util_s.str.is_string(r_vec.get(0))){
            auto ex_var = l_vec.get(0);
            if(ex_var->get_small_id()==r_vec.get(1)->get_small_id()){
                auto ex_str = r_vec.get(0);
        ...
        if(num_r<2)return;
        if(util::is_str_variable(l_vec.get(num_l-1),m_util_s) &&m_util_s.str.is_string(r_vec.get(num_r-1))){
            auto ex_var = l_vec.get(num_l-1);
            if(ex_var->get_small_id()!=r_vec.get(num_r-2)->get_small_id())return;
            auto ex_str = r_vec.get(num_r-1);
            auto rex_star = m_util_s.re.mk_star(m_util_s.re.mk_to_re(ex_str));
            expr_ref_vector exs(m);
            exs.push_back(expr_ref(m_util_s.re.mk_in_re(ex_var,rex_star),m));
            zstring s = to_app(ex_str)->get_decl()->get_parameter(0).get_zstring();
    ...
    // abc = def, bc = hg -> ahg = def
    // todo: only focus on prefix and suffix now and efficient method on substr needs to be added.
    std::pair<bool, bool> theory_str_noodler::check_pos(const expr_ref& pattern, const expr_ref& text){
        expr_ref_vector p_vec(m),t_vec(m);
        m_util_s.str.get_concat(pattern.get(),p_vec);
        m_util_s.str.get_concat(text.get(),t_vec);
        //todo: what will happen if they are eqaul
        if(p_vec.size()<t_vec.size()){
            bool first=true,second = true;
            size_t nlen= p_vec.size(),mlen = t_vec.size();
            for(size_t pos = 0;pos<nlen;pos++){
                if(t_vec[pos].get()->get_small_id()!=p_vec[pos].get()->get_small_id()){
                    first=false;
                    break;
                }
            }
            for(size_t pos = 1;pos<=nlen;pos++){
                if(t_vec[mlen-pos].get()->get_small_id()!=p_vec[nlen-pos].get()->get_small_id()){
                    second=false;
                    break;
                }
            }
    ...
    std::vector<std::pair<expr_ref,expr_ref>> theory_str_noodler::build_new_eq(const std::pair<bool,bool> ll,const expr_ref_vector& l_vec,const expr_ref_vector& r_vec,const expr_ref_vector& l_t,const expr_ref_vector& r_t){
        std::vector<std::pair<expr_ref,expr_ref>> tot_;
        if(ll.first){
            //r+lt[x:]=rt
            expr_ref_vector tmp_vec(m);
            for(const auto& ex:r_vec)tmp_vec.push_back(expr_ref(ex,m));
            for(size_t pos = l_vec.size();pos<l_t.size();pos++)tmp_vec.push_back(expr_ref(l_t[pos],m));
            ...
            //lt[:x]+r=rt
            expr_ref_vector tmp_vec(m);
            for(size_t pos = 0;pos<l_t.size()-l_vec.size();pos++)tmp_vec.push_back(expr_ref(l_t[pos],m));
            for(const auto& ex:r_vec)tmp_vec.push_back(expr_ref(ex,m));
            tot_.push_back(std::make_pair(expr_ref(m_util_s.str.mk_concat(tmp_vec,nullptr),m)
                ,expr_ref(m_util_s.str.mk_concat(r_t,nullptr),m)));
        }
    ...
    void theory_str_noodler::eq_extend(const expr_ref& left_side,const expr_ref& right_side){
        ...
        for(const auto& ex: m_word_eq_todo){
            ...
            if(!m_util_s.str.is_concat(left_text.get())|| !m_util_s.str.is_concat(right_text.get()))
                return;
            //todo: optimize?
            expr_ref_vector l_t(m),r_t(m);
            m_util_s.str.get_concat(left_text.get(),l_t);
            m_util_s.str.get_concat(right_text.get(),r_t);

            auto ll = check_pos(left_side,left_text),lr = check_pos(left_side,right_text),
                rl =  check_pos(right_side,left_text),rr = check_pos(right_side,right_text);
            auto tmp = build_new_eq(ll,l_vec,r_vec,l_t,r_t);
            for(auto i:tmp)tot_.push_back(i);
            tmp = build_new_eq(lr,l_vec,r_vec,r_t,l_t);
            for(auto i:tmp)tot_.push_back(i);
            tmp = build_new_eq(rl,r_vec,l_vec,l_t,r_t);
            for(auto i:tmp)tot_.push_back(i);
            tmp = build_new_eq(rr,r_vec,l_vec,r_t,l_t);
            for(auto i:tmp)tot_.push_back(i);
            
            ll = check_pos(left_text,left_side);lr = check_pos(left_text,right_side);
            rl =  check_pos(right_text,left_side);rr = check_pos(right_text,right_side);
            // std::swap(l_vec,l_t);std::swap(r_vec,r_t);
            tmp = build_new_eq(ll,l_t,r_t,l_vec,r_vec);
            for(auto i:tmp)tot_.push_back(i);
            tmp = build_new_eq(lr,l_t,r_t,r_vec,l_vec);
            for(auto i:tmp)tot_.push_back(i);
            tmp = build_new_eq(rl,r_t,l_t,l_vec,r_vec);
            for(auto i:tmp)tot_.push_back(i);
            tmp = build_new_eq(rr,r_t,l_t,r_vec,l_vec);
            for(auto i:tmp)tot_.push_back(i);
    ...
    void theory_str_noodler::check_lyndon(const expr_ref& left_side,const expr_ref& right_side){
        if(!m_util_s.str.is_concat(left_side.get())|| !m_util_s.str.is_concat(right_side.get()))
            return;
        expr_ref_vector l_vec(m),r_vec(m);
        m_util_s.str.get_concat(left_side.get(),l_vec);
        m_util_s.str.get_concat(right_side.get(),r_vec);
        if(l_vec.size()!=r_vec.size() || l_vec.size()<=1)return;
        expr_ref_vector l_text(m);
        auto size_l = l_vec.size(),size_r = r_vec.size();
        for(size_t pos=0;pos<size_l*2;pos++){
            l_text.push_back(l_vec[pos%size_l].get());
        } 
        //todo: change to kmp if bf is slow.
        for(size_t pos=1;pos<size_l;pos++){
            bool fl = false;
            for(size_t posj=0;posj<size_r;posj++){
                if(r_vec.get(posj)->get_small_id()!=l_text.get(pos+posj)->get_small_id()){
                    fl=true;
                    break;
                }
            }
            if(!fl){
                ...
                bool in_set = false;
                for(const auto &ep:lyndon_pairs){
                    unsigned h1= get<0>(ep),h2= get<1>(ep);
                    ...
                    if(ha==h1){
                        expr* tmp_r=m_util_s.str.mk_concat(get<3>(ep),nullptr);
                        expr* eb=m_util_s.str.mk_concat(b_vec,nullptr);
                        //b_vec tmp_r = tmp_r b_vec
                        // ctx.mark_as_relevant(m.mk_eq(m_util_s.str.mk_concat(eb,tmp_r),m_util_s.str.mk_concat(tmp_r,eb)));
                        // add_axiom({mk_literal()});
                        expr_ref tmp_eq (m.mk_eq(m_util_s.str.mk_concat(eb,tmp_r),m_util_s.str.mk_concat(tmp_r,eb)),m);
                        m_rewrite(tmp_eq);
                        if(m.is_false(tmp_eq.get())){
                            expr_ref unsat_core(m.mk_true(), m);
                            unsat_core = m.mk_not(unsat_core);
                            ...
                            auto r_res = to_app(tmp_eq)->get_arg(1);
                            m_word_eq_todo.push_back({expr_ref(l_res,m), expr_ref(r_res,m)});
                            add_axiom({mk_literal(tmp_eq.get())});
                        }

                    }else{
                        expr* tmp_l = m_util_s.str.mk_concat(get<2>(ep),nullptr);
                        expr* ea = m_util_s.str.mk_concat(a_vec,nullptr);
                        //a_vec tmp_l = tmp_l a_vec
                        m_word_eq_todo.push_back({expr_ref(m_util_s.str.mk_concat(ea,tmp_l),m), expr_ref(m_util_s.str.mk_concat(tmp_l,ea),m)});
                        add_axiom({mk_literal(m.mk_eq(m_util_s.str.mk_concat(ea,tmp_l),m_util_s.str.mk_concat(tmp_l,ea)))});
                    
                    }
                }
                if(!in_set)
                lyndon_pairs.push_back(std::make_tuple(ha,hb,a_vec,b_vec));
                break;
            }
        }
    }
