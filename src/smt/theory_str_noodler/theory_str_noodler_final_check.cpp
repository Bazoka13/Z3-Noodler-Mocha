...
namespace smt::noodler {
    ...
    Formula theory_str_noodler::get_formula_from_relevant(const std::set<mata::Symbol>& alph) {
        ...
        for (const auto &we: this->m_word_eq_todo_rel) {
            ...
            // STRACE(str,tout<<"-----> "<<inst.to_string()<<"\n";);
            ...
            for (const auto &side : pred.get_params()) {
                for (const auto &var_or_literal : side) {
                    if (var_or_literal.is_variable() && aut_assignment.find(var_or_literal) == aut_assignment.end()) {
                        // STRACE(str,tout<<var_or_literal.get_name()<<"is varible\n";);
    ...
    void theory_str_noodler::solve_len_eq(){
        ...
        for(const auto& eq : this->m_word_eq_todo_rel) {
            ...
            std::unordered_map<unsigned int,unsigned int> co_l,co_r;
            int fl = 0;
            ...
            for(size_t i =0;i<num_l;i++){
                auto ex = l_vec.get(i);
                if(util::is_str_variable(ex,m_util_s)){
            ...
            for(size_t i =0;i<num_r;i++){
                ...
                if(util::is_str_variable(ex,m_util_s)){
                    ...
                    sum_r += m_util_s.str.max_length(ex);
                }else{
                    fl=1;
                    break;
                }
            }
            if(co_l.size()>1||co_r.size()>1)fl=1;
            for(auto pi:co_l){
                if(co_r.find(pi.first)==co_r.end())fl=1;
            }
            if(fl)continue;
            //co_l + sum_l = co_r+sum_r
            if(co_l.empty()&&co_r.empty()&&sum_l!=sum_r){
                ...
                auto col=co_l[var_id],cor=co_r[var_id];
                if(col<cor){
                    std::swap(col,cor);
                    std::swap(sum_l,sum_r);
                }
                col-=cor;
                sum_r-=sum_l;
                if(sum_r<0){
