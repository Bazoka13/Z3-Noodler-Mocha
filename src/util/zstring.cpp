...
zstring zstring::replace(zstring const& src, zstring const& dst) const {
    ...
    int pos = idxOf(*this,src,0);
    if(pos!=-1){
        for (unsigned i = 0; i < pos; ++i) {
            result.m_buffer.push_back(m_buffer[i]);
        }
        result.m_buffer.append(dst.m_buffer);
        for (unsigned i = pos+src.length(); i < length(); ++i) {
            result.m_buffer.push_back(m_buffer[i]);
        }
    }else{
        for (unsigned i = 0; i < length(); ++i)result.m_buffer.push_back(m_buffer[i]);
    }
    // bool found = false;
...
int zstring::last_indexof(zstring const& other) const {
    ...
    zstring const& this_reverse = reverse();
    int pos = idxOf(this_reverse,other_reverse,0);
    if(pos>=0){
        pos-=other.length()-1;
        return pos;
    }
